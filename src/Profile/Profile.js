...
function Profile() {
  ...
  const [Walletconnection, setWalletconnection] = useState(provider.isConnected);
  async function phantom_balance() {
    ...
    if (phantom.isConnected !== false) {
      ...
      let _balance = await connection.getBalance(wallet)
        .then(function(data) {
          console.log("Wallet Balance: " +parseFloat(data) / solanaWeb3.LAMPORTS_PER_SOL);
          return data;
        })
 ...
 <main>
  ...
  <div className='absolute top-3 right-[5%] z-10'><Logout/></div>
  <div className='relative grid content-center gap-3 w-9/12 h-56 rounded-[20px] bg-yellow'>
    <div className='w-full flex justify-between'>
      <img alt='profile' className='absolute h-36 left-6 bottom-[-25px] bg-white rounded-[100px] ml-5 w-36 object-fill' src={`http://localhost:8000/images/${user.image}`} />
  ...
  <div className='relative grid content-center w-9/12  rounded-[20px]'>
    <div className='w-full flex pt-5 justify-between'>
      ...
      <div>
        {(Walletconnection) === false ? <button className='bg-red text-white font-light py-1 px-5 rounded-[10px] mr-2' onClick={() => LinkWallet()}>Link Wallet</button> : <button className='bg-green text-white font-light py-1 px-5 rounded-[10px] mr-2'>Wallet Connected</button>}
      <Link to={"/EditProfile"} >
        <button className='bg-green text-white font-light py-1 px-5 rounded-[10px]' >Edit profile</button>
        </Link>
        <button className='bg-green text-white font-light py-1 px-2 rounded-[10px]' onClick={()=>phantom_balance()}>Check Sol Balance</button>
        <button className='bg-green text-white font-light py-1 px-2 rounded-[10px]' onClick={()=>checkBalance()}>Check BT Balance</button>
      </div>
    </div>
  ...
  <div className='relative grid gap-3 w-full mt-4 h-56 '>
    <div className='w-full flex-wrap flex justify-start'>
      ...
      <div className='grid bg-white ml-4 w-56 h-28 justify-items-center text-xs rounded-[10px]'>
        ...
        <p className='pl-4 overflow-hidden wrap'>Extra feature for those who like to experience more </p>
